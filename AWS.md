...
7 インスタンスの起動、停止、再起動、終了、休止

停止　→　データが消える

再起動　→　データが保持される

8  スポットインスタンスが中断された　→　最初1時間に終了また停止された場合、請求しない
...
ネットワークインターフェース当たりセキュリティグループ最大数は5まで。
...
10.1 プロトコル  →　IKE , IPsec , Tunnel , BGP（オプション)
...
RDS プロビジョンドIOPS　→　パフォーマンス改善

RDS  リードレプリカ　→ 　パフォーマンス改善]

7 マルチAZ配置

稼働中と同じリージョンにスタンバイに登録できる

8 自動バックアップ

9 ストレージエンジン　クラッシュんから　回復できるはInooDB
...
6.S3 Glacierオブジェクトを取り出す場合、3~5時間をかける
...
インスタンススケールアウトした後、特定の期間が経過するまでスケーリングアクティビティを中断する(ディフォルトは300秒)

5.HTTPS/SSLで通信する場合、SSL証明書をインストールする場所

6.タイムアウトのディフォルトの設定は60秒
...
3 Snowball →　ペタバイト規模のデータをAWSクラウドに転送するソリューション

4 ECU →　Elastic Computer Unit 安定した通りのCPU能力を提供する
